 ...
 */
  ...
  this.subject.masterOpened(data, function() {
    ...
    case O.masterState.GONE:  // Link to master can be closed
      return;
    case O.masterState.WAITING:
    case O.masterState.BUSY:
      that.home(data.home);
...
exports.close = function() {  // {{{2
  this.subject.setMasterState(O.masterState.BUSY);
  reopen(this);
};
...
exports.error = function(err) {  // {{{2
  ...
  this.subject.setMasterState(O.masterState.WAITING);
 ...
 */
  ...
  that.subject.setMasterState(O.masterState.GONE);
  delete that.subject;
  return;
}
...
function onGw(subject, err, gw) {  // {{{2
  ...
  if (err) {
    ...
    subject.setMasterState(O.masterState.WAITING);
  ...
  function onConnected(is) {  // {{{3
    ...
    if (is) {
      subject.setMasterState(O.masterState.BUSY);
