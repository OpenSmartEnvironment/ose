 ...
 */
  ...
  function onGw(err, resp) {  // {{{3
    ...
    if (err) {
      // TODO: Instead of timeout setup some handler on gw, getGw should be renamed to getConnectedGw or added parameter (connected {Boolean})
      // When there is not connected gw, call `getGw(false, onDisconnectedGw)`

      setTimeout(onTime, 10000);
      return;
    }
