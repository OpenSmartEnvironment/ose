 ...
 * @param req.dval {Object} New entry data value
 *
 * @param [socket] {Object} Client socket
 *
 * @method new
 ...
 */
  ...
  s.lhsLink2(socket, function(lhs) {
    switch (lhs) {
    case LHS.WAITING:
      O.link.error(socket, O.error(s, 'DISCONNECTED', 'Shard is not connected to it\'s master'));
      return;
    case LHS.AT_HOME:
      s.newEntry(req.id, req.kind, req.val, function(err, entry) {
        ...
        O.link.close(socket, {rev: entry.drev});
        return;
      });
      return;
    case LHS.MASTER:
...
exports.map = function(req, socket) {  // {{{2
/**
 * Handler called when a slave shard requests a map.
 * TODO: rename to "map"
 *
 * @param req {Object} Request to be sent to `shard.getMap()`
 * @param socket {Object} Slave entry socket
 *
 * @method map
 * @handler
