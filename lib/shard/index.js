...
exports.embryo = function(id, kind, dval, cb) {  // {{{2
  if (! id) {
    id = this.nextTime();
...
exports.newEntry = function(id, kind, dval, cb) {  // {{{2
/**
 ...
 * @param [dval] {Object} Optional entry data value
 ...
 */
  ...
  this.db.put(e.id, e.kind.name, e.drev, e.data, function(err) {
    if (err) {
      e.error(err);
      cb(err);
      return;
 ...
 */
  ...
  if (res.setup(kind)) {
    if (this.db) {
      this.db.put(res.id, res.kind.name, res.drev, res.data, O._.noop)
    }
 ...
 */
  ...
  deps.add('shards', 'peers', function(cb) {  // {{{3
    ...
    that.db = O.new(data.db.id || data.db)(that);
    that.db.config(data.db, function(err, props) {
      if (err) {
        O.log.error(err);
        cb('shards');
        return;
