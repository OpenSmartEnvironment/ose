...
exports.pipeFrom = function(eid, stream) {  // {{{2
/**
 * Pipe from entry blob to `stream`
 *
 * @param eid {Number|String} Entry id
 * @param stream {Object} Stream to be piped to
 *
 * @method pipeFrom
 */

  if (this.schema.pipeFrom) {
    return this.schema.pipeFrom(eid, stream);
  }

  this.read(eid, function(err, src) {
 ...
 */
  ...
  if (this.schema.read) {
    ...
    cb(O.error(this, 'Schema read is not implemented', this.schema.name));
  }
};
...
exports.cleanCache = function(cb) {  // {{{2
  this.setBusy(function(err, that) {
    if (err) {
      return cb && cb(err);
    }

    O.async.forEachOf(that.cache, function(entry, key, cb) {
      entry.remove(cb);
    }, function(err) {
      if (err) {
        that.error(err);
...
exports.getDataDir = function() {  // {{{2
/**
 * Return directory with shard data
 *
 * @returns {String} Path to shard data
 *
 * @method getDataDir
 */

  return O.dataDir + '/' + this.space.name + '/' + this.id;
};
