...
exports.entryLookupIsIdentified = function(val, ident) {  // {{{2
  switch (O.typeof(val)) {
  case 'number':
    return val === ident.eid;
  case 'string':
    return val === ident.eal;
  case 'object':
    return (val.shard === this.id || val.shard === this.alias) && (val.entry === ident.eid || val.entry === ident.eal);
  case 'array':
    return val[0] === ident.eid && val[1] === ident.sid;
  }
