...
var EntryReadable = M.class('../entry/readable');
...
/**
 * Gets called after all shards in the current space are initialized
 *
 * @method afterInit
 */
...
/**
 * Executes a callback after the shard has been initialized, but only in
...
//  console.log('SHARD GET', entry);
  ...
  if (entry in this.cache) {
    ...
    cb(M.log.todo('Entry is created and waiting for the kind', {shard: this.alias, entry: entry.id}));
    return;
  }
...
//      new EntryMaster(this.cache[entry], req, socket);
    ...
    getFromDb(this, entry, function(err, resp) {
      if (err) {
        Ose.link.error(socket, err);
      } else {
        resp.linkSlave(req, socket);
      }
    });
    return;
  }
...
//  console.log('SHARD GET VIEW', this.sid || this.alias, params);
  ...
  if (this.db && this.db.getView) {
    this.db.getView(params, socket);
    return;
  }
...
exports.readStream = function(entry, cb) {  // {{{2
  if (this.atHome()) {
    if (this.db) {
      this.db.getStream(entry, cb);
      return;
    }

    cb(Ose.error(this, 'MISSING_DB', 'Unable to read a stream! There is no db assigned to the shard.'));
    return;
  }
  ...
  this.linkMaster(function(err, sm) {
    ...
    var ws = sm.link.ws;
    if (! ws.isConnected()) {
      Ose.link.error(cb, Ose.error(ws, 'NOT_CONNECTED'));
      return;
    }

    var res = new EntryReadable();
    cb(null, res);
    ws.tx({
      type: 'readStream',
      lid: sm.link.lid,
      entry: entry,
      newLid: ws.addLink(res),
    });
    return;
  });
  return;
...
function getFromDb(that, entry, cb) {  // {{{2
  if (! that.db) {
    cb(Ose.error(that, 'ENTRY_NOT_FOUND', entry));
    return;
  }

  var res = new Entry(that, entry);

  that.db.get(entry, function(err, kind, data) {
    if (err) {
      res.remove();
      cb(err);
      return;
