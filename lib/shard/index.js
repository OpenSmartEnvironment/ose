 ...
 *
 * @param ident {Number|String|Object} Entry identification
 * TODO
 *
 * @method track
 */
 ...
 * @param req.key {Object} Exact key
...
/ **
 * Establish a link to the master shard.
 *
 ...
 * /
  ...
  if (! this.db) {
    return cb(O.error(this, 'Database is not specified'));
  }
...
exports.send = function(eid, name, data, socket) {  // {{{2
/ **
...
//  console.log('ENTRY SEND', eid, name, data);

  var that = this;

  if (this.isAtHome()) {
    return this.get(eid, function(err, entry) {
...
exports._link = function(eid, drev, srev, socket) {  // {{{2
  var that = this;

//  console.log('SHARD _LINK', eid, drev, srev, typeof socket);

  if (eid in this.cache) {  // Entry is available in cache
    return this.cache[eid].awaitBusy(function(err, entry) {
      if (! L.canClose(socket)) return true;
      if (err) return L.error(socket, err);

      return entry.linkBusy(drev, srev, socket);
    });
  }

  if (! this.isAtHome()) {  // Entry is not in cache and we are not at home
    return new Entry(this, eid).linkInit(drev, srev, socket);
  }

  if (! this.maps.all) {  // We are at home, but there is no database available TODO: Is it possible?
    return L.error(socket, entry.remove('ENTRY_NOT_FOUND'));
  }

  var entry = new Entry(this, eid);  // Put entry into cache

  var sync = this.maps.all.get(eid, function(err, val) {  // Try to get entry data from database
    sync = true;
    ...
    if (! entry.setupKind(val[0], val[1], val[2])) {
      return L.error(socket, entry.goneError());
    }

    return entry.linkInit(drev, srev, socket);
  });
  return sync;
};
...
exports.newEntry = function(kind, val, cb) {  // {{{2
/**
 * Create a new entry and add it to `this.cache` and optional database backend. Can be used only "at home".
 *
 ...
 * /
  ...
  if (! this.isAtHome) {
    throw O.log.error(this, 'NOT_AT_HOME', 'Can\'t create entry in another peer than shard\'s home!', {alias: alias, kind: kind, data: data});
  }

  var that = this;

  var id = new ArrayBuffer(8);
  var vid = new UInt32Array(buffer);
  vid[0] = this.space.psid;
  vid[1] = ++this.lastEid;
  ...
  this.db.put(e.id, [e.kind.name, e.drev, e.dval], function(err) {
    console.log('NEW ENTRY CREATED', id, err);
...
exports.saveEmbryo = function(entry, val, cb) {  // {{{2
  ...
  this.lhsLink(function(err, lhs) {
    ...
    case LHS.AT_HOME:
      ...
      that.db.put(entry.id, [entry.kind.name, kind, rev, val], function(err) {
        if (err) {
          cb(err);
          return;
        }
    ...
    case LHS.MASTER:
      L.send(that.master, 'new', {
        ...
        delete entry.embryo;
        entry.updateData(resp.drev, val, 'saveEmbryo');
        return;
      });
      return;
    }

    throw O.log.error(this, 'Unhandled lhs', lhs);
  });
};
 ...
 * /
  ...
  this.space.shards[this.id] = this;

  return true;
};

exports.masterOpened = function(data, cb) {  // {{{2
/ **
 *
 * @method masterOpened
 * @internal
 * /
  ...
  for (var key in this.shard.cache) {
    ...
    if (entry.master) entry.awaitMaster(O._.noop);
  }
