  ...
  O.async.series([
    ...
    function(cb) {  // Prepare data, mark entries busy {{{3
      O.async.forEachOf(that.data, function(item, key, cb) {
        ...
        case 'patch':
          item.dval = Diff.mergeCopy(item.entry.dval, item.patch);
...
//        console.log('UPDATE MAP', name, vals);
  ...
  function prepAdd(entry) {  // {{{3
    ...
    for (var name in shard.scope.maps) {
      var map = shard.scope.maps[name];
...
/**
 * Add entry patch to the transaction.
 *
 * @param id {Buffer} Entry id
...
exports.add = function(kind, val) {  // {{{2
/**
 * Add new entry to the transaction.
 *
 * @param kind {Object|String} Entry kind
 * @param [val] {Object} Optional `entry.dval`
 *
 * @return {Object||false} Return new entry or false when entry can't be created
 * @method add
 */

  var e = new Entry(this.shard, this.shard.space.spid * 256 * 256 * 256 * 256 + ++this.shard.lastEid);
  ...
  case 'string':
    ...
    return this.shard.find(ident, function(err, entry) {
      ...
      if (entry.id in that.data) {
        return cb(O.error(this.shard, 'ENTRY_NOT_FOUND'));
      }
