...
exports.reuse = function(socket) {  // {{{2
  switch (socket._state) {
  case exports.state.SPLIT:
  case exports.state.CLOSED:
  case exports.state.ERROR:
    delete socket._state;
    return;
  }

  throw Ose.error(socket, 'INVALID_STATE', 'Invalid socket state', socket._state);
};

exports.isOpened = function(socket) {  // {{{2
  return socket &&
    socket.link &&
    socket._state === exports.state.OPENED
  ;
};
 ...
 */
  ...
  if (! socket) {
    error(socket, 'MISSING_SOCKET', 'Response socket is not defined');
    return;
  }

  if (! client) {
    error(socket, 'MISSING_SOCKET', 'Client socket is not defined');
    return;
  }

  if (socket._state)  {
    error(socket, 'INVALID_STATE', 'Response socket is not waiting for open', socket._state);
    return;
  }

  if (client._state)  {
    error(client, 'INVALID_STATE', 'Client socket is not waiting for open', client._state);
    return;
  }

  if (socket.link) {
    error(socket, 'DUPLICIT_LINK', 'Response socket has already defined property `link`');
    return;
  }

  if (client.link) {
    error(socket, 'DUPLICIT_LINK', 'Client socket has already defined property `link`');
    return;
  }

  if (typeof client.open !== 'function') {
    error(client, 'MISSING_HANDLER', 'Missing socket handler', 'open');
    return;
  }

  socket._state = exports.state.OPENED;
  client._state = exports.state.OPENED;
  ...
  function error(subject, code, message, data) {
    var err = Ose.error(subject, code, message, data);

    if (socket.link !== client) {
...
exports.post = function(socket, name, data, client) {  // {{{2
  if (socket._state !== exports.state.OPENED) {
    error(Ose.error(socket, 'CLOSED', 'Socket is not opened', {name: name, data: data}));
    return;
  }
  ...
  case 2:
    ...
    break;
  default:
    error(Ose.error('INVALID_ARGS', 'Invalid argument count', arguments));
    return;
  }

  if (name in exports.forbiddenNames) {
    error(Ose.error(socket, 'INVALID_HANDLER', 'Command is forbidden', {name: name, data: data}));
    return;
  }
  ...
  error(Ose.error(socket, 'CLOSED', 'Socket has no _link neather _ws', {name: name, data: data}));
  return;

  function link(l) {  // {{{3
    if (typeof l[name] !== 'function') {
      error(Ose.error(socket, 'MISSING_HANDLER', 'Command not handled by link', {name: name, data: data}));
      return;
    }

    if (l.state !== exports.state.OPENED) {
      error(Ose.error(socket, 'CLOSED', 'Linked socket is not opened', {name: name, data: data}));
      return;
    }
  ...
  function ws() {  // {{{3
    if (! ws.isConnected()) {
      error(Ose.error(ws, 'DISCONNECTED', 'WebSocket is not connected', {name: name, data: data}));
      return;
    }
    ...
    if (client) {
      d.newLid = Tx.addLink(this._ws, client);
    }
  ...
  function error(err) {  // {{{3
    if (client) {
      exports.error(client, err, 0);
    } else {
      exports.error(socket, err, 0);
    }
  }
 ...
 */
  ...
  function doIt(s) {
    ...
    case exports.state.OPENED:
      ...
      exports.error(s, Ose.error(s, 'MISSING_HANDLER', 'Missing socket handler', {handler: 'close', data: data}));
      return;
    }

    throw Ose.error(s, 'INVALID_STATE', 'Invalid socket state for close', s._state);
  }
};
 ...
 */
  ...
  function doIt(s) {
    ...
    case undefined:
    case exports.state.OPENED:
      s._state = exports.state.ERROR;

      switch (err.code) {
      case 'DISCONNECTED':
      case 'UNREACHABLE':
        s._state = exports.state.SPLIT;
    ...
    throw Ose.error(s, 'INVALID_STATE', 'Invalid socket state for error', s._state);
  }
};

exports.wsClose = function(data) {  // {{{2
//  console.log('WS CLOSE', this._lid);
...
exports.wsError = function(err) {  // {{{2
  if (this._lid && this.ws && this.ws.isConnected()) {
    this.ws.txError(this._lid, err);
  ...
  for (var i = 0; i < h.length; i++) {
    ...
    default:
      if (key in exports.forbiddenNames) {
        throw Ose.error(socket, 'HANDLER_FORBIDDEN', 'Handler is forbidden', key);
      }
...
exports.bindHandlers = function(socket, handlers) {  // {{{2
  for (var i = 0; i < handlers.length; i++) {
    ...
    if (name in socket) {
      return Ose.error(socket, 'DUPLICIT_HANDLER', 'Duplicit handler name', name);
    }

    if (exports.forbiddenNames.indexOf(name) >= 0) {
      return Ose.error(socket, 'HANDLER_FORBIDDEN', 'Handler is forbidden', key);
    }
...
function txCommand(name, data, socket) {  // {{{2
  ...
  var cmd = {
    type: 'command',
    lid: this._lid,  // Already created target socket lid.
