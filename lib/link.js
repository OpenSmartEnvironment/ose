...
//  console.log('LINK ERROR', socket._lid, err.code, err.message);
  ...
  if (! (err instanceof Error)) {
    err = O.error.apply(O, arguments.slice(1));
  }
 ...
 */
  ...
  if (socket._state !== SS.OPENED) {
    return cb(O.error(socket, 'CLOSED', 'Socket is not opened', {name: name, data: data}));
  }
  if (typeof cb !== 'function') {
    return cb(O.error(socket, '`cb` must be a function', {name: name, data: data}));
  }
  if (exports.forbiddenNames.hasOwnProperty(name)) {
    return cb(O.error(socket, 'Command is forbidden', {name: name, data: data}));
  }
  ...
  if (! mate || '_ws' in socket) {
    return cb(O.error(socket, 'Can\'t send data to socket'));
  }
  ...
  if (typeof mate[name] !== 'function') {
    return cb(O.error(mate, 'Missing socket handler', {name: name, data: data}));
  }
 ...
 */
  ...
  if (typeof mate[name] !== 'function') {
    error(O.error(mate, 'MISSING_HANDLER', {name: name, data: data}));
    return false;
  }
  ...
  function error(err) {  // {{{3
    ...
    if (client && exports.canClose(socket)) {
      exports.error(socket, err);
      return;
    }
...
exports.bind = function(socket) {  // {{{2
  return function(err, data) {
    if (err) {
      exports.error(socket, err);
    } else {
      exports.close(socket, data);
    }
  }
};
...
//  console.log('LINK MOCK', req.newLid);
