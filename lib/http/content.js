...
var M = require('ose').class(module, C);
var Fs = require('fs');
...
/**
 * Adds a module among scripts in <body>.
 *
 ...
 */
  ...
  if (ext) {
    if (ext !== '.js') {
      M.log.unhandled('Only possible extension for module is ".js".');
      return;
    }
...
/**
 * Adds a JavaScript file to scripts in <body>.
 *
 ...
 */
  ...
  if (ext) {
    if (ext !== '.js') {
      M.log.unhandled('Only possible extension for js is ".js".', name, ext);
      return;
    }
...
/**
 * Adds a CSS file to scripts in <body>.
 *
 ...
 */
 ...
 var ext = Path.extname(name);
  ...
  if (ext) {
    if (ext !== '.css') {
      M.log.unhandled('Only possible extension for stylesheet is ".css".');
      return;
    }
 ...
 */
  ...
  for (var key in this.modules) {
    resp.write('  <script src="' + this.name + '/' + key + '.js' + '"></script>');
  };
};
...
//  console.log('RESPOND', this.path, name, ext);
  ...
  function onStat(err, stat) {  // {{{3
    if (err) {
      M.log.unhandled('File stat error!', {path: path, err: err});
  ...
  function onRead(err, data) {  // {{{3
    var module;

    if (err) {
      M.log.unhandled('File read error!', {path: path, err: err});
      resp.statusCode = 500;
      resp.end();
    ...
    if (ext === '.js') {
      module = params.module || that.modules[name];
    }
...
//          console.log('RESPOND MODULE', module, name);
          ...
          data = 'window.ose("' + module + '", "' + that.name + '/' + name + '", function(exports, require, module, __filename, __dirname) {' + data + '\n});';
        }
        break;
      case '.map':
      ...
      case '.mp4':
        ...
        break;
      default:
        M.log.unhandled('Uknown file extension, "Content-Type" is not specified', path);
    }

//    resp.setHeader('Content-Length', data.length);  // WARNING: Some files are truncated when this header is sent, propably due to UTF characters in module.
