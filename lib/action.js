...
exports.serialize = function() {  // {{{2
  ...
  function subject(value) {
    ...
    if (typeof value.getIdent === 'function') {
...
exports.deserialize = function(data, cb) {  // {{{2
  ...
  this.peer = O.peers[data.peer];
  this.name = data.name;
  this.data = data.data;

  O.findEntry(data.source, function(err, entry) {
    if (err) {
      O.log.unhandled('ENTRY NOT FOUND', data.source);
    } else {
      that.source = entry;

      O.findEntry(data.target, function(err, entry) {
        if (err) {
          O.log.unhandled('ENTRY NOT FOUND', data.target);
        } else {
          cb && that.cb(cb);
          that.post(entry);
        }
...
exports.logError = function() {  // {{{2
  this.on('error', function(err) {
    O.log.unhandled('Action response error', {
      action: this.name,
      data: this.data,
      source: this.source.getIdent(),
      target: this.target && (typeof this.target.getIdent === 'function' ? this.target.getIdent() : this.target),
      error: err
    });
  });
};
...
exports.chain = function(name, data, target, cb) {  // {{{2
  O.log.todo();  // Not yet tested!

  if (! Action) {
    Action = O.getClass('ose/lib/action');
  }
...
exports.post = function(value) {  // {{{2
  if (value) {
    ...
    if (! this.target) {
      O.log.unhandled('Target is not defined', action.serialize());
      return this;
    }
  }

  O.async.setImmediate(send.bind(null, this), 0);
  return this;
};
...
//  console.log('********SENDING ACTION', that.serialize());
  ...
  if (((typeof that.source) === 'object') && O.isSuper('ose/lib/entry', that.source)) {
    testTarget();
  } else {
    O.findEntry(that.source, onSource);
  }
  ...
  function testTarget() {  // {{{3
    if (((typeof that.target) === 'object') && O.isSuper('ose/lib/entry', that.target)) {
      doIt();
    } else {
  ...
  function onTarget(err, target) {  // {{{3
    switch (err) {
      ...
      case undefined:
        ...
        break;
      default:
//        O.log.unhandled('ACTION ON TARGET ERROR', {error: err, action: that.name, source: that.source.getIdent(), target: that.target});
...
function sendTarget(that) {  // {{{2
  ...
  function onResp(err, data) {  // {{{3
    switch (err) {
      ...
      case undefined:
        that.emit('done', data);
//        O.log.unhandled('ACTION RESPONSE', data);
        break;
...
//        console.log('********POSTPONING ACTION TILL PEER IS CONNECTED', that.target.shard.master, that.target.shard.peer);
      ...
      default:
        ...
        that.emit('error', err);
//        O.log.unhandled('ACTION RESPONSE ERROR', err);
    }
  };
...
//      console.log('****************PEER IS CONNECTED, SENDING ACTION', that.serialize());
      ...
      O.log.unhandled('INVALID PEER EVENT', data);
    }
  };
...
//    O.log.unhandled('ACTION DONE HANDLER');
