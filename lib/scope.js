...
exports.getHomeSchema = function(config) {  // {{{2
  return require('./schema/cache');
};

exports.getRemoteSchema = function(config) {  // {{{2
  return require('./schema/remote');
};
 ...
 */
  ...
  if (shard.isAtHome()) {
    shard.schema = this.getHomeSchema(config.schema);
  } else {
    shard.schema = this.getRemoteSchema(config.schema);
  }
