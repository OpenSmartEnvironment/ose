 ...
 */
  ...
  if (kind.name in this.kinds) {
    throw O.log.error(this, 'alreadyExist', kind.name);
  }
...
exports.initMaps = function(shard, cb) {  // {{{2
  O.async.forEachOf(m, function(val, key, cb) {
    val.init(shard, cb);
  }, cb);
};
...
//        console.log('SCOPE SHARD GET MAP', counter.count, shard.scope.name, kind, shard.alias);
      ...
      shard.getMap(params, function(err, val) {
        switch (err && err.message) {
        case null:
        case undefined:
          if (val.map && val.map.length) {
            space[shard.id] = val.map;
          }
          break;
        ...
        default:
          console.log('ERROR RETRIEVING MAP', shard.alias, kind, params, val);
          O.log.error(err);
        }
