 ...
 * @param [ident.sal] {String} Shard's alias
 ...
 * @param ident.eid {Number|String} Entry id
 ...
 * /
  ...
  // Try to find requested shard
  return O.findShard(ident, function(err, shard) {
    if (! O.link.canOpen(socket)) return;
    if (err) return O.link.error(socket, err);

    shard._link(ident.eid, drev, srev, socket);
  });
});
...
//  console.log('OSE POST ENTRY', ident, name, data);
  ...
  O.findShard(ident, function(err, shard) {
    ...
    shard.post(ident.eid, name, data, socket);
    return;
  });
});
