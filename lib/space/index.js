...
/**
 * Find shards based on `filter`. Calls `cb()` for each shard found and
 * then calls `done()`
 *
...
exports.notifyHome = function() {  // {{{2
/**
 * Send list of local shards to space home
 *
 * @method notifyHome
 * @internal
 */
  ...
  if (resp.length) {
    O.link.send(this.master, 'notify', {
      peer: O.here.name,
      shards: resp,
    });
  }
...
function findShard(that, ident, cb) {  // {{{2
  ...
  function onFind(err, val) {  // {{{3
    if (err) {
      cb(err);
      return;
    }

    if (val.sid in that.shards) {
      cbShard(that.shards[val.sid], cb);
      return;
    }

    shard = new Shard();
    if (
      shard.setup(that, val.sid) &&
      shard.setupConfig(val.home, val.scope, val.alias) &&
      shard.setupDone()
    ) {
      cb(null, shard);
      return;
