...
// Public {{{1
function C(owner, field, layout, val) {  // {{{2
/**
 ...
 */
  ...
  throw O.log.error(this, 'Invalid field layout', layout);
}
 ...
 */
  ...
  if (this.edit) {
    if (val !== this.edit.value) return;

    this.stopEdit(this);
  }

  this.update(patch, orig);
  return;
  */
};
