...
window.COMPONENTS_BASE_URL = '/ose-gaia/bower_components/';
 ...
 * Provides CommonJS Require emulation. Registers module wrapped by function `init`.
 *
...
/**
 * Setups OSE framework and intializes plugins.
 *
 * @method run
 */
  ...
  var mod = cache('ose/lib/browser');

  O = Cache.ose.exports.module(mod);
  O.extend('now', now);
  O.extend('exit', exit);
  Cache.ose.exports.setup();

  window.O = O;  // TODO: For easy debugging from the browser use for example ose.O.getSpace('example.org')

  O.run(mod.require('ose/config'));
};
...
//  console.log('CACHE', id);
  ...
  var mod = Modules[id];
  if (! mod) {
    var err = O.error('MODULE_NOT_FOUND', 'Module was not found: "' + id + '"', id);
    throw err;
  }
