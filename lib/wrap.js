 ...
 * @param [method] {Function} When no method is specified, default initialization method or constructor is used.
 *
 * @method inherited
 */
...
//  Object.defineProperty(this, 'exports', {value: this.module.exports, configurable: true});
...
//    case 'lib':
      break;
    default:
      const u = this.require(name.join('/'));
...
function prepareConstructor(that) {  // {{{2
  return function() {
    ...
    if (this instanceof that.module.exports) {
      ...
      let res = new that.module.exports;
...
function callInit(that, obj, args) {  // {{{2
  ...
  if (that.super.O.hasOwnProperty('prototype')) {
...
function preparePrototype(that) {  // {{{2
  if (! that.module.loaded) {
    throw O.log.error('Class module loading is not yet finished!');
  }

  defineO(that, that.module.exports.prototype);
...
function inherits(that, init, superClass) {  // {{{2
  ...
  case 'function':
    if (! that._.isEmpty(init.prototype)) {
      throw O.log.error(that, 'INVALID_ARGS', 'There is something defined on `init.prototype`, but `init` should be ordinary function', init);
    }
    that._init = init;
    break;
  default:
  ...
  if (! that._init) {
    if (that.super.O) {
      that._init = that.super.O._init;
    } else {
      that._init = that.super;
    }
  }
  */
};
