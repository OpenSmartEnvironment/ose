...
/**
 * Call `method` in `obj.prototype` with `args`
 *
 * @param obj {Object}
 * @param method {Function}
 * @param [arg]* {*} Optional arguments to send to obj's init function.
 ...
 */
  ...
  while (w) {
    ...
    case 'class':
      ...
      break;
    default:
      throw O.log.error(this, 'Invalid module type', w.type);
    }
  }
 ...
 */
  ...
  return 'Wrap: ' + this.type + ' ' + (this.module && this.module.id);
};
...
exports.prototype.kind = function(id, name, deps) {  // {{{2
/**
 * Create new kind instance
 *
 * @param id {String} Module id
 * @param name {String} Kind name
 ...
 */
  ...
  if (! ('_scope' in this)) {
    throw O.log.error(this, 'Missing scope');
  }

  this.object(id)(this.scope, name, deps);
};
...
exports.prototype.scopeExist = function(name) {  // {{{2
/**
 * Tests whether scope exists
 *
 * @param name {String} Scope name
 *
 * @return {Boolean} Whether scope exists
 *
 * @method scopeExists
 */
...
/**
 * Create a new scope instance of a given name or returns an existing one.
 *
...
exports.prototype.getKind = function(ident) {  // {{{2
/**
 * Try to find a kind defined by an identification object.
 *
 * @param ident {Object} Identification object.
 * @param ident.kind {String} Kind name
 * @param ident.scope {String} Scope name
 *
 * @returns {Object} `Kind` instance
 *
 * @method getKind
 */

  var scope = this.scopes[ident.scope];

  return scope && scope.kinds[ident.kind];
};
...
function getCtor() {  // {{{2
  ...
  if (! ('__ctor' in this)) {
    throw O.log.error(this, 'Wrap is not a class', this.type);
  }
