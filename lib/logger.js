 ...
 * @readme
 *
 * To log errors and messages, each module should at first create `O.log` instance by calling
 * `O.logger(context)`. The context is an identifier of the logging
 * namespace. `O.logger()` either returns an existing `O.log`
 ...
 * error is logged, `subject.identify()`, if defined, is used
 ...
 *
 * When calling any callback with an error response, sending an error to a link, or throwing an exception, `Error` instance created by `O.error()` should be used.
 *
 ...
 */
  ...
  case 3:
    if (count) {
      ...
      if (! err.subject._suppressError) {
        err.subject._suppressError = {};
      }
      var c = err.subject._suppressError[err.code] || 1;
      if (c > count) {
        return;
      }
      if (c === count) {
        console.log('Suppressing following error, already logged', c, 'times');
      }
      err.subject._suppressError[err.code] = c + 1
    }

    this.log('error', message, {code: err.code, message: err.message, data: err.data});
    return err;
  }
