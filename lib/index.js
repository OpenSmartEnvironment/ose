 ...
 */
  ...
  if (! val.name) {
    throw O.error(this, 'INVALID_CONFIG', 'Missing local instance name', val);
  }
  ...
  deps.add('core', function(cb) {
    if (! (val.space in O.spaces)) {
      throw O.error('INVALID_CONFIG', 'Space was not found', val.space);
    }

    O.here.space = O.spaces[val.space];
    O.here.space.peers[val.name] = O.here;
