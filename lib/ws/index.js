...
//  console.log('WS CLOSE', this.peer && this.peer.name, err);
  ...
  if (links) {
    ...
    for (var key in links) {  // Close all socket, send "error" to relay sockets
      ...
      case 'object':
        ...
        case SS.OPENED:
          ...
          if (O.isSuper('EventEmitter', socket)) {
            O.async.setImmediate(function() {
              if (this.listenerCount('error')) {
                ...
                O.log.error(this, 'Missing socket error handler', 'error');
              }
            }.bind(socket));
            break;
          }
          ...
          O.log.error(socket, 'Missing socket error handler', 'error');
          break;
        default:
  ...
  try {
    ...
    */
    ...
    case 'ping':
      break;
    case 'chunk':
      console.log('TX >>>>>>', this.peer && this.peer.name, JSON.stringify({type: val.type, lid: val.lid}));
      break;
    default:
