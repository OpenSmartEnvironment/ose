...
//  console.log('WS CLOSE', this.peer && this.peer.name, err);
  ...
  for (var key in links) {
    ...
    case SS.OPENED:
      ...
      O.log.error(socket, 'Missing socket split or error handler');
      break;
    default:
 ...
 */
  ...
  if (! val) {
    throw O.error(this, 'Invalid request', val);
  }

  if (! dontVerifyPeer && ! this.peer) {
    throw O.error(this, 'UNEXPECTED', 'Trying to send data to WebSocket wrapper without peer!', val);
  }

  try {
    /*
    if (! (this.ws && this.ws.readyState === 1)) {
      throw O.error(this, 'UNEXPECTED', 'Trying to send data to closed WebSocket!', val);
    }
    */
