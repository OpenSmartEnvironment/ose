...
// Public {{{1
O.extend('readConfig', function(val) {  // {{{2
/**
 * Read main configuration object (`val`), creates all plugins based
 * on this object and processes all dependencies, and finally emit the
 * `done` event. See the [plugins component].
 *
 * @param val {Object} Main configuration object
 *
 * @method readConfig
 */
  ...
  for (var key in val) {
    ...
    if (typeof v !== 'object') {
      throw O.error('Plugin configuration is not an object', {key: key, config: v});
    }
...
//    console.log('CONFIGURING', key, v, typeof plugin.config);
    ...
    if (plugin.O) {
      ...
      case 'object':
        ...
        break;
      default:
        throw O.error(wrap, 'Invalid module', v);
      }
    }
