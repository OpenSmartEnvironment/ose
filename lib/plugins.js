 ...
 *
 * @todo
 * All plugins are registered to the plugins module. This module can
 * prepare configurations for the OSE browser instances as
 ...
 */
  ...
  O.async.setImmediate(function() {  // {{{3
    ...
    for (var key in val) {  // Create and register all plugins.
  ...
  //    console.log('CONFIGURING', key, v, typeof plugin.config);
      ...
      if (wrap) {
        if (wrap.isClass()) {
          plugin = new wrap.exports();
        } else if (wrap.isSingleton()) {
  ...
  O.async.setImmediate(function() {  // Configure current dependencies {{{3
    ...
    if (exec) O.async.setImmediate(function() {
 ...
 */
  ...
  for (var key in exports.plugins) {
    ...
    data.push('exports[' + JSON.stringify(key) + '] = ' + JSON.stringify(browserConfig(p.plugin, p.data, req), null, 2) + ';\n');
  };
...
// Private {{{1
function browserConfig(plugin, data, req) {  // {{{2
  ...
  case 'function':
    ...
    plugin.browserConfig(result, data, req);
    return result;
  case 'object':
