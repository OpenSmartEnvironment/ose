 ...
 */
  ...
  case 2:
    ...
    case 'string':
      var m = this.O.requireChain(handler);
      if (typeof m === 'function') {
        this.on(name, m);
        return;
      }

      if (m.O && m.O.type === 'class') {
        this.commands[name] = onClass(m.O.ctor);
        return;
      }

      break;
    case 'function':
      if (handler.prototype.O && handler.prototype.O.type === 'class') {
        this.commands[name] = onClass(handler);
        return;
      }
