 ...
 * [shards] are called [entries]. Each shard is a key/value store
 * indexed by `entry.id` using LevelDB.
 *
 * Each [entry] is of a certain [kind]. [Kinds] define the properties
 * and behaviour of [entries]. Kinds are namespaced using [schemas].
 *
 * Each [shard] belongs to a [space] that act as the shard's
 * namespace. Each shard is tied to [schema] and can contain only
 * entries of kinds from that [schema].
 *
 * Kind hierarchy:
 * * schema
 *   * kind
 ...
 *
 *
 * Entries within shards can be indexed using [maps]. Each map is a
...
exports.expandIdent = function(ident, shard) {  // {{{2
  switch (typeof ident) {
  case 'number':
    return [ident, shard.id, shard.space.name];
  case 'string':
    return {
      entry: ident,
      shard: shard.id,
  ...
  case 'object':
    if (Array.isArray(ident)) {
      switch (ident.length) {
      case 2:
        return [ident[0], ident[1], shard.space.name];
      case 3:
        return ident;
      }

      return O.error('INVALID_IDENT', ident);
    }

    return {
      entry: ident.entry,
      shard: ident.shard || shard.id,
