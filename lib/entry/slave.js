 ...
 */
  ...
  var t = {
    dc: 0,  // Whether the tracking of `entry.dval` by the slave is changed by this command and send data to the slave
    sc: 0,  // Whether the tracking of `entry.sval` by the slave is changed by this command and send data to the slave
    dr: 0,  // Whether the tracking of `entry.dval` is changed by this command and request it from the master
    sr: 0,  // Whether the tracking of `entry.sval is changed by this command and request it from the master
  };
 ...
 */
  ...
  e.lhsLink2(socket, function(lhs) {
    switch (lhs) {
    case LHS.AT_HOME:
      if (O._.isEmpty(req.patch)) {
        L.error(socket, O.error(e, 'Empty patch'));
        return;
      }
