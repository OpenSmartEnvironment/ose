...
//      console.log('ENTRY MASTER AFTER TRACK', entry.id, that.slaveId);
      ...
      if (! err) {
        M.log.todo('Entry has no kind but entry.track (entry.linMaster) returned no error!', entry, arguments);
        return;
      }
...
//  console.log('ENTRY MASTER CLOSE', e.id, this.slaveId);
  ...
  if (! Ose._.isEmpty(e.slaves)) {  // Entry has some other slaves
    e.track(
      ...
      this.strack ? -1 : 0,
      function(err) {
        if (err && (err.code !== 'DISCONNECTED')) M.log.error(err);
      }
    );
    return;
  }
...
//  console.log('ENTRY MASTER ERROR', this.entry && this.entry.id, err);
