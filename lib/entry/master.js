 ...
 */
  ...
  if (entry._state === O.subjectState.READY) {
    entry._state = O.subjectState.BUSY;
  }
  ...
  entry.shard.awaitSteadyMaster(function(err) {
    ...
    if (err) {
      if (entry._state !== O.subjectState.GONE) {
        throw O.log.error(entry, 'INVALID_STATE');
      }
      return;
    }

    if (entry.shard.masterState === O.masterState.WAITING) {
      return entry.setMasterState(O.masterState.WAITING);  // TODO: Shard must notify entries with `.master` about reconnect
    }
 ...
 */
  ...
  case O.subjectState.INIT:
    if (entry.kind) {
      ...
      entry.setupKind(resp.kind, resp.drev, resp.dval);
    }
 ...
 */
  ...
  if (entry._state === O.subjectState.INIT) {
    if (entry.kind) {
      entry.setMasterState(O.masterState.WAITING);
      return entry.setup();
    }
  ...
  return entry.setMasterState(O.masterState.WAITING);
}
