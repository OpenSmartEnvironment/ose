...
//  console.log('ENTRY POST TO', ident, name, data);
  ...
  case 'object':
    ...
    if (! ident.id) {
      L.error(socket, O.error(this, 'Invalid entry `ident`', ident));
      return;
    }
    ...
    findShard(this.shard, ident, function(err, shard) {
      ...
      shard.send(ident.id, name, data, socket);
      return;
    });
    return;
  }
...
//  console.log(JSON.stringify(patch));
  ...
  if (this.lhs !== LHS.AT_HOME) {
    switch (this.lhsInited(O.dummyFn)) {
    case undefined:
      return;
    case false:
 ...
 */
  ...
  case 'object':
    if (! (kind instanceof Kind)) {
      throw O.error(this, 'Invalid kind', {kind: kind});
    }
    this.kind = kind;
    break;
  default:
...
//    console.log('ENTRY CLOSING MASTER', this.id);
  ...
  this.trackMaster(dc, sc, O.dummyFn);
  return;
};
...
/*
 * Update entry tracking. Master must be connected. Call `cb()` after successull tracking was setup and entry was updated or on error.
 */
...
//  var that = this;
  ...
  if (cb) throw O.error(this, '`entry.track()` no more supports `cb` argument', cb);
