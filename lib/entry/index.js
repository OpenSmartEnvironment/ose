 ...
 */
  ...
  case 'object':
    ...
    if ('shard' in ident && (
      ident.shard !== this.shard.id || ident.shard !== this.shard.alias
    )) return false

    return ident.entry && (
      ident.entry === this.id ||
      typeof ident.entry === 'string' && ident.entry === (entry.dval && entry.dval.alias)
    );
  case 'array':
    switch (ident.length) {
    case 2:
 ...
 */
  ...
  switch (typeof kind) {
  case 'string':
    kind = this.shard.schema.kinds[kind];
    // NO BREAK
  case 'object':
...
//  console.log('ENTRY COMMAND', this.id, name, data);
  ...
  if (! L.canClose(socket)) {
    O.log.warning('Socket is invalid or can\'t be closed, exit silently', {entry: this.identify(), command: name, data: data});
    return;
  }
