 ...
 */
  ...
  if (this.shard.isAtHome()) {
    ...
    if (this.sval && ! this.srev) this.srev = O.nextTime();
  }
 ...
 */
  ...
  function link() {  // {{{3
    ...
    var sc = srev && ! that.stc ? 1 : 0;  // Whether to newly track sval
  ...
  L.send(this.master, 'track', r, function(err, resp) {
    ...
    if (that.lhs === LHS.TRACKING) {
      if (! that.master || typeof that.master.tracking !== 'number') {
        throw O.error(that, 'entry.sval is LHS.TRACKING, but master has no `tracking`');
      }
 ...
 */
  ...
  if (! this.sval) this.sval = {};
  merge(this.sval, patch);
