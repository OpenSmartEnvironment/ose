 ...
 */
  ...
  if ('data' in this) {
    if (! this.drev) this.drev = 1;
  } else {
    this.data = {};
    if (! this.drev) this.drev = 0;
  }

  if ('state' in this) {
    if (! this.srev) this.srev = 1;
  } else {
    this.state = {};
    if (! this.srev) this.srev = 0;
  }
 ...
 */
  ...
  try {
    ...
    M.log.notice('Error in entry remove', this.identify());
    M.log.error(err);
  }
...
//  console.log('RESPOND', this.id, req, resp);
