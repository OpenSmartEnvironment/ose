 ...
 *
 * The `state` JSON object, unlike `dval`, can
 ...
 *
 * The `dval` and `state` objects can contain any valid JSON excluding
 ...
 *
 * @aliases entry entries dval data state statesOfEntries
 *
 * @class ose.lib.entry
...
/** drev {{{2
 * Timestamp in microseconds of last dval update
 *
 * @property drev
 * @type Number
 */
 ...
 */
  ...
  if (id in shard.cache) {
    this.error(this, 'DUPLICIT_ENTRY', 'Entry with this id already exist in shard');
  } else {
    shard.cache[id] = this;
  }
};
 ...
 *
 * @param patch {Object} State patch
...
//    delete this.stc;
  ...
  if (this.shard.cache[this.id] === this) {
 ...
 */
  ...
  if (this.shard.isAtHome()) {
    ...
    if (this.dval && ! this.drev) this.drev = O.nextTime();
 ...
 */
  ...
  function link() {  // {{{3
    var dc = drev && ! that.dtc ? 1 : 0;  // Whether to newly track dval
...
/**
 * Creates a response with `entry.dval` or `entry.state`.
 *
 ...
 */
  ...
  if (this.dtc < 0) {
    throw O.error(this, 'Invalid data track count `dtc`', {dtc: this.dtc, data: dd});
  }
  if (this.stc < 0) {
    throw O.error(this, 'Invalid state track count `stc`', {stc: this.stc, state: sd});
  }
 ...
 */
  ...
  var val = JSON.parse(JSON.stringify(this.dval));
  merge(val, patch);

  if (
    this.shard.db &&
    ! this.embryo &&
    typeof this.shard.db.put === 'function'
  ) {
    this.shard.db.put(this.id, this.kind.name, rev, val, done);
  } else {
    done();
  }
  ...
  function done(err) {
    ...
    O.log.debug('Data patched', JSON.stringify({entry: that.id, patch: patch}));

    cb && cb();

    that.emit('dpatch', patch, src);

    if (that.slaves) for (var key in that.slaves) {
      L.send(that.slaves[key], 'patch', {drev: rev, dpatch: patch, src: src}, null, true);
    }
  }
};
...
function clean(patch, orig) {  // Alter "patch" - remove properties that are same as in "orig". {{{2
  if (! orig) return;
